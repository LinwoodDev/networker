...
abstract class NetworkingConnection {
  ...
  FutureOr<bool> _handleData(NetworkingConnection connection, dynamic data) {
    ...
    final message = NetworkingMessage(
...
abstract class NetworkingServer extends NetworkingConnection {
  ...
  void send(
    ...
    final receivers = clients.isEmpty
        ? this.clients
        : clients.map((id) => getClient(id)).toList();
    for (var client in receivers) {
      client?.send(service: service, event: event, data: data);
    }
  }
  ...
  void broadcastRoom(
    ...
    clients
        ...
        .forEach((client) =>
            client.send(service: identifier, event: event, data: data));
  }
...
abstract class NetworkingClient extends NetworkingIdentity {
  ...
  @override
  FutureOr<void> joinRoom(String room) =>
      send(service: 'room', event: 'join', data: room);
...
abstract class NetworkingClientConnection extends NetworkingIdentity {
  ...
  FutureOr<bool> joinRoom(String room) async {
    ...
    if (success) {
      await send(service: 'room', event: 'joined', data: room);
    }
    return success;
  }
  ...
  FutureOr<bool> leaveRoom(String room) async {
    ...
    if (success) {
      await send(service: 'room', event: 'left', data: room);
    }
    return success;
  }
...
class NetworkingMessage {
  ...
  void reply(String data) {
    connection.send(service: service, event: event, data: data);
  }
}
